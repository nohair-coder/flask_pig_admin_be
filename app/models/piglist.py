...
class PigList(db.Model):
    ...
    animalnum = db.Column(db.String(12), unique=True)  # 种猪号（对种猪的自定义代指）
    earid = db.Column(db.String(12), unique=True)  # 耳标号 id
    ...
    def get_all(self):
        ...
        :return:
        '''
        return PigList.query.with_entities(PigList.animalnum, PigList.earid).all()
    ...
    def exit_one_station(self, exit_time):
        ...
        :return:
        '''
        PigList.query.filter_by(stationid=self.stationid).update({
            'exit_time': exit_time,
        })
        db.session.commit()
